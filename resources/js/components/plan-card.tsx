...
const MostPopularBadge = () => (
    <div className="absolute -top-3 right-8 transform">
        <div className="bg-primary text-primary-foreground rounded-full px-3 py-1 text-xs font-semibold shadow-sm">Most Popular</div>
    </div>
);
...
const PricingDisplay = ({ plan }: { plan: Plan }) => (
    <div className="mt-4">
        ...
        <span className="text-muted-foreground ml-2">{plan.price === 0 ? 'forever' : plan.price < 0 ? '' : '/ year'}</span>
    </div>
);

const FeatureItem = ({ feature }: { feature: string }) => (
    <li className="flex items-start">
        <svg xmlns="http://www.w3.org/2000/svg" className="text-primary mt-0.5 mr-3 h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path
                fillRule="evenodd"
...
export default function PlanCard({ plan, isButtonDisabled = false, billing = false }: PlanCardProps) {
    return (
        <div
            className={`bg-card relative rounded-xl border p-8 shadow-sm ${
                plan.most_popular ? 'border-primary border-2 shadow-md' : 'border-border'
            }`}
        >
            ...
            <ul className="mb-8 space-y-4">
                ...
                {plan.features?.map((feature, index) => <FeatureItem key={index} feature={feature} />)}
            </ul>
